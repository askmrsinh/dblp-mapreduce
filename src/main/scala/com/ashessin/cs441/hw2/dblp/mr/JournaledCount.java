...
public class JournaledCount extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new JournaledCount(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Dblp Top Journals");
        job.setJarByClass(JournaledCount.class);
    ...
    public static class DblpMapper extends Mapper<Text, PublicationWritable, Text, IntWritable> {
        ...
        protected void map(Text key, PublicationWritable publication, Context context) throws IOException, InterruptedException {
            journal = new Text(publication.getJournal());
    ...
    public static class DblpReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
         ...
         * @param context
         */
        protected void reduce(Text journal, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            int sum = 0;
            for (IntWritable val : values) {
                sum += val.get();
            }
            result.set(sum);
            context.write(journal, result);
        }
    }
}
