...
public class CoauthorshipBinCount extends Configured implements Tool {
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new CoauthorshipBinCount(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Dblp Coauthorship Bin Count");
        job.setJarByClass(CoauthorshipBinCount.class);
    ...
    public static class DblpMapper extends Mapper<Text, PublicationWritable, IntWritable, IntWritable> {
        ...
        private IntWritable numAuthors = new IntWritable();
        ...
        protected void map(Text key, PublicationWritable publication, Context context) throws IOException, InterruptedException {
            numAuthors = new IntWritable(publication.getAuthors().size());
            context.write(numAuthors, ONE);
        }
    }
    ...
    public static class DblpReducer extends Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
         ...
         * @param context
         */
        protected void reduce(IntWritable numAuthors, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            int sum = 0;
            for (IntWritable val : values) {
                sum += val.get();
            }
            result.set(sum);
            context.write(numAuthors, result);
        }
    }
}
